...
export default {
    ...
    getters: {
        autoTransferListFilter(state) {
            state.autoTransferList.filter((x) => {
                x.price > 100000
            })
        }
    },
    // store 영역에서 state의 값을 직접 동기적으로 변경하는 함수, actions 내부에서 사용됨
    mutations: {
        setAutoTransferList(state, payload) {
            state.autoTransferList.push(payload)
        },
        clearAutoTransferList(state) {
            state.autoTransferList.length = 0
        }
    },
    ...
    actions: {
        ...
        // actions의 함수에서 state나 mutations, dispatch를 호출하기 위해서는
        // 해당 함수의 첫 번째 매개변수를 중괄호를 열고 state(state), commit(mutation), dispatch(actions) 식으로 사용해야 함  
        async fetchList({ state, commit }) {
            ...
            try {
                ...
                let response = await axios.get('http://localhost:8088/api/autoTransferList', {}, {
                    headers: {
                ...
                for (let newData of response.data) {
                    ...
                    commit('setAutoTransferList', temp)
                    temp = {}
                }
                ...
                // 콘솔 확인용
                console.log('state 확인: ', state.autoTransferList)
