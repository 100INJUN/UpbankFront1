<template>
    ...
    <div>
        ...
        <table>
            ...
            <tbody>
                <tr v-for="autoTransfer in autoTransferList" v-bind:key="autoTransfer.autoTransfer" @click="moveDetail(autoTransfer.autoTransfer)">
                    <td> {{ autoTransfer.account }} </td>
                    <td> {{ autoTransfer.autoSenderbank }} </td>
                    <td> {{ autoTransfer.autoSenderaccount }} </td>
                    <td> {{ autoTransfer.autoBalance}} </td>
                    <td> {{ autoTransfer.autoOutdate }} </td>
                    <td> {{ autoTransfer.autoRegistdate}}</td>
                    <td> {{ autoTransfer.autoExpirationdate}}</td>
                    <td> {{ autoTransfer.autoInplace}}</td>
                    <td> {{ autoTransfer.autoLastday}}</td>
...
export default {
    ...
    setup() {
        ...
        const autoTransferList = computed(() => {
            // vue 파일에서 state를 받는 방법
            // store.state.(모듈이름).(state이름)
            return store.state.listAutoTransfer.autoTransferList
        })
        ...
        const deleteProduct = async (id) => {
            ...
            await store.dispatch('listAutoTransfer/fetchList')
        }
